...
class Speedometer @JvmOverloads constructor(
    ...
    private var size = 0 // Overall size of the view (width and height)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val desiredSize = resolveSizeAndState(size, widthMeasureSpec, 0)
        setMeasuredDimension(desiredSize, desiredSize)
        size = desiredSize // Ensure the size matches the measured width
        recalculateDependentSizes()
    }


    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        super.onSizeChanged(w, h, oldw, oldh)
        size = w // Ensure the size matches the width
        recalculateDependentSizes()
    }

    /**
     * Sets the overall size (width and height) of the Speedometer.
     */
    fun setSize(newSize: Int) {
        size = newSize
        requestLayout() // Triggers onMeasure and onSizeChanged
    }

    /**
     * Recalculates sizes for dependent attributes.
     */
    /**
     * Recalculates dependent sizes for drawing components.
     */
    private fun recalculateDependentSizes() {
        ...
        tickBorderRect.set(
            ...
            borderSize + TICK_MARGIN,
            size - borderSize - TICK_MARGIN,
            size - borderSize - TICK_MARGIN
        )
        recalculateTextSizes()
        invalidate() // Trigger a redraw after recalculating
    }

    private fun recalculateTextSizes() {
        val baseTextSize = size / 7f // Adjust this scaling factor to fit your design

        paintSpeed.textSize = baseTextSize * 2f // Larger for speed display
        paintMetric.textSize = baseTextSize * 0.5f // Smaller for metric text
        paintTickText.textSize = baseTextSize * 0.4f // Smaller for tick labels
    }
    ...
    private fun obtainStyledAttributes(attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        try {
            with(typedArray) {
                maxSpeed = getInt(R.styleable.Speedometer_maxSpeed, maxSpeed)
                borderSize = getDimension(R.styleable.Speedometer_borderSize, borderSize)
                textGap = getDimension(R.styleable.Speedometer_textGap, textGap)
                metricText = getString(R.styleable.Speedometer_metricText) ?: metricText
                borderColor = getColor(R.styleable.Speedometer_borderColor, borderColor)
                fillColor = getColor(R.styleable.Speedometer_fillColor, borderColor)
                textColor = getColor(R.styleable.Speedometer_textColor, borderColor)
            }
        } finally {
            typedArray.recycle()
        }
    ...
    private fun renderSpeedAndMetricText(canvas: Canvas) {
        canvas.drawTextCentred(
            speed.toString(),
            width / 2f,
            height / 2f - paintSpeed.textSize / 5,
            paintSpeed
        )
        canvas.drawTextCentred(
            metricText,
            width / 2f,
            height / 2f + paintSpeed.textSize / 3 + textGap,
            paintMetric
        )
    }
    ...
    companion object {
        ...
        private const val MAJOR_TICK_SIZE = 40f
